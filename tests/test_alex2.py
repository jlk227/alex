...
import tempfile
import pytest
from alex2 import alex2


@pytest.fixture
def client():
    db_fd, alex2.app.config['DATABASE'] = tempfile.mkstemp()
    client = alex2.app.test_client()
    with alex2.app.app_context():
        alex2.init_db()

    yield client

    os.close(db_fd)
    os.unlink(alex2.app.config['DATABASE'])
